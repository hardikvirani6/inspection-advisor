...
@property (assign, nonatomic, getter = isFullSize) BOOL fullSize;
 ...
 * \return NO if this representation does not contain a file, otherwise YES.
 */
- (BOOL)getContentPath:(NSOperationQueue *)handlerQueue requestPriority:(NSOperationQueuePriority)priority onComplete:(void (^)(NSString *path))completionBlock onError:(void (^)(NSError *error))errorBlock;
