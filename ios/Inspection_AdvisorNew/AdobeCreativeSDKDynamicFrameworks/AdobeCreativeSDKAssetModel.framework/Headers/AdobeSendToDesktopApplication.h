...
/**
 * Sends an NSData object to the specified desktop application. The object is opened as a file with 
 * the name and extension provided. This is an asynchronous command.
 *
 * This method is intended for cases where the data to send is not already in a file, e.g., when 
 * downloading a file from the web and saving it as NSData. If the file is already on the Creative 
 * Cloud use @c `sendAsset:toApplication:withName:onSuccess:onProgress:onCancellation:onError:`. If 
 * the file is local on the device, use 
 * @c `sendLocalFile:withType:toApplication:withName:onSuccess:onProgress:onCancellation:onError:`.
 *
 * Note that if the length of @c `data` is too large, this method might fail. In cases where the 
 * size of @c `data` is too large, please store the data in a temporary file and use the 
 * @c `sendLocalFile:withType:toApplication:withName:onSuccess:onProgress:onCancellation:onError:` 
 * method to send the file to the desired application on the desktop.
 *
 * @param data              The data object to be sent to the desktop application.
 * @param mimeType          The data object mime type. For mime types known to the SDK, this will 
 *                          create the extension of the file opened on the destination application.
 * @param application       Destination application.
 * @param name              The file name on the destination application. It must include an 
 *                          extension consistent with the application.
 * @param successBlock      Block to be executed on a successful send operation.
 * @param progressBlock     Optionally track the upload progress.
 * @param cancellationBlock Optionally be notified of a cancellation on upload.
 * @param errorBlock        Block to be executed when on error.
 ...
 * @return Request token. It can be used to cancel or change the priority of the request.
 *
 * @see sendAsset:toApplication:withName:onSuccess:onProgress:onCancellation:onError:
 * @see sendLocalFile:withType:toApplication:withName:onSuccess:onProgress:onCancellation:onError:
 */
