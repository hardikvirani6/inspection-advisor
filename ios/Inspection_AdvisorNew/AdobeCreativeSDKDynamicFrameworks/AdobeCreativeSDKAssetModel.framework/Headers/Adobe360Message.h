...
/*
 * A data structure that describes the attributes of a media asset embedded in an Adobe360Message
 */
...
/**
 * A data structure that describes a generic media asset embedded in an Adobe360Message
 */
 ...
 *
 * @note the specified type of the asset must match the type of the DCX element or
 * the assetWithData factory methods will throw an NSInvalidArgumentException
 */
...
/**
 * Allocates, initializes and returns an instance of @see Adobe360Asset.
 *
 * @param data A reference to the asset @see data
 * @param type The Internet media type describing the content type of the asset
 * @param path The relative path used to locate the asset within the message and its serialized representation
 *
 * @return A newly allocated instance of @see Adobe360Asset.
 */
 ...
 *
 * @param data A reference to the asset @see data
 * @param descriptor Additional details describing the asset
 *
 * @return A newly allocated instance of @see Adobe360Asset.
 */
...
/**
 * An Adobe360Message is an immutable object that provides an interface that allows access to the contents of
 * a 360 workflow message used to exchange data between applications participating in a 360 deg Workflow.
 *
 * @note Adobe360Message objects are constructed using the Adobe360MessageBuilder class and by the readFrom class methods.
...
/**
 * Creates a message from the serialized message data that exists at filePath
 *
 * @param path Path to message data (DCX snapshot, composite or archive file)
 * @param queue The operation queue on which the completion handler will be called
 * @param handler Called upon completion with a references to the resulting message or error, depending
...
/**
 * Creates a message from an in-memory message archive
 *
 * @param messagedData NSData object containing the archive bytes
 * @param queue The operation queue on which the completion handler will be called
 * @param handler Called upon completion with a references to the resulting message or error, depending
 * on whether the operation was successful
 *
...
/**
 * Creates a message from a DCX composite representation
 *
...
/**
 * Write the message to a DCX snapshot
 *
 ...
 * @param queue The operation queue on which the completion handler will be called
 * @param handler Called upon completion with a non-nil file path or error, depending
 * on whether the operation was successful
 *
...
/**
 * Write the message to a DCX composite at the specified path
 *
 * @param compositePath Path to the new composite
 * @param queue The operation queue on which the completion handler will be called
 * @param handler Called upon completion with a non-nil composite or error, depending
 * on whether the operation was successful
 *
 * @return An NSProgress object that may be used to cancel the packaging operation and report progress events
 */
...
/**
 * Create an in-memory representation of a message in a single buffer
 *
 * @param queue The operation queue on which the completion handler will be called
 * @param handler Called upon completion with a non-nil message data or error, depending
 * on whether the operation was successful
 *
 * @return An NSProgress object that may be used to cancel the packaging operation and report progress events
 */
...
/**
 * Writes an scalar valued input to the specified path
 *
 * @param name The name that the input valued is associated with
 * @param path The path to write to
 * @param errorPtr An error object if there is any
 *
 * @return YES if an input asset with name existed, NO otherwise
 *
 * @note This method may be used with both atomic and DCX assets.  In the case
...
/**
 * Writes an scalar valued output to the specified path
 *
 * @param name The name that the output valued is associated with
 * @param path The path to write to
 * @param errorPtr An error object if there is any
 *
 * @return YES if an output asset with name existed, NO otherwise
 *
 * @note This method may be used with both atomic and DCX assets.  In the case
 * of a DCX element then path specifies the directory into which the corresponding composite will be written.
 */
...
/**
 * Writes an asset stored at a specific index in a named input array to the specified path.
 *
 * @param name The name that the output valued is associated with
 * @param index The index that the asset is stored
 * @param path The path to write to
 * @param errorPtr An error object if there is any
 *
 * @return YES when an array of asset with the specified name existed, NO otherwise
 *
 * @note This method may only be used for  inputs that refer to an array of assets, which may consist of both atomic and DCX elements.
 ...
 *
 * @param name The name that the output valued is associated with
 * @param index The index of an asset in the asset array with name
 * @param path The path to write to
 * @param errorPtr An error object if there is any
 *
 * @return YES when an array of asset with the specified name existed, NO otherwise
 *
 * @note This method may only be used for outputs that refer to an array of assets, which may be both atomic or DCX elements.
 * If the asset is a DCX element then the path specifies the directory into which the corresponding composite will be written.
...
/**
 * Get an NSData object with the bytes that comprise an atomic asset refered by a scalar named input.
 *
 * @param name a scalar named input
 * @param errorPtr An error object if there is any
 *
 * @return an NSData object with the bytes that comprise an atomic asset refered by a scalar named input
 *
 * @note This method may be used for action assets that have both on-disk and in-memory representations
...
/**
 * Get an NSData object with the bytes that comprise an atomic asset refered by a scalar named output.
 *
 * @param name a scalar named output
 * @param errorPtr An error object if there is any
 *
 * @return an NSData object with the bytes that comprise an atomic asset refered by a scalar named output
 *
...
/**
 * Get an NSData object with the bytes that comprise an atomic asset that is stored at a specific index
 * in a named input array.
 *
 * @param name the name of the input assets
 * @param index the index of an asset in the input assets array
 * @param errorPtr An error object if there is any
 *
 * @return an NSData object with the bytes that comprise an atomic asset that is stored at a specific index
 * in a named input array
 *
 * @note This method may only be used for named inputs that refer to an array of assets.  The assets themselves may have either
 ...
 *
 * @param name the name of output assets
 * @param index the index of an asset in the output assets array
 * @param errorPtr An error object if there is any
 *
 * @return an NSData object with the bytes that comprise an atomic asset that is stored at a specific index
 * in a named output array
 *
 * @note This method may only be used for outputs that refer to an array of assets.  The assets themselves may have either
 * on-disk or in-memory representations within the message but this method may not used for assets that are represented as DCX elements.
 * The number of elements in a given output array correspond to the size of the NSArray for the named output that is available through
 * the 'outputDescriptors' property of the this object.
 */
...
/*
 * Returns a DCX Element of the scalar input asset with the specified name is represented as such
 *
 * @param name the specified name
 * @param errorPtr An error object if there is any
 *
 * @return a DCX Element
 */
- (AdobeDCXElement *)elementForInputWithName:(NSString *)name withError:(NSError **)errorPtr;

/**
 * Get a DCX Element of the scalar output asset with the specified name is represented as such
 * 
 * @param name the specified name
 * @param errorPtr An error object if there is any
 *
...
/**
 * Get a DCX Element if the asset stored at a specific index in an input array is represented as such
 *
 * @param name the name of the input assets
 * @param index the index of an asset in the input array
 * @param errorPtr An error object if there is any
 *
 * @return a DCX Element
 *
 * @note The number of elements in a given input array correspond to the size of the NSArray for the named input that is available through
 * the 'inputDescriptors' property of the this object.
 */
- (AdobeDCXElement *)elementForAssetInInputArrayWithName:(NSString *)name atIndex:(NSUInteger)index withError:(NSError **)errorPtr;

/*
 * Get a DCX Element if the asset stored at a specific index in an output array is represented as such
 *
 * @param name the name of the output assets
 * @param index the index of an asset in the output array
 * @param errorPtr An error object if there is any
 *
 * @return a DCX Element
 *
 * @note The number of elements in a given output array correspond to the size of the NSArray for the named output that is available through
 * the 'outputDescriptors' property of the this object.
 */
...
/**
 * Helper method that deletes the files referenced by the message
 *
 * Depending on the method used to instantiate the message object, these files may comprise the snapshot,
 ...
 *
 * @note Subsequent calls to writeAssetWithName, dataForAssetWithName, and pathToImmutableAssetWithName will
