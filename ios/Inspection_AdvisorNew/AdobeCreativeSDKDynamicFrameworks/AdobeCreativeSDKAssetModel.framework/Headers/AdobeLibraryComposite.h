...
#import "AdobeCollaborationType.h"
...
/** Whether or not this is a public library. */
@property (readonly, nonatomic) BOOL isPublic;

/** Whether or not this is a local library that never gets synced to the cloud. Local libraries are not meant to be shown to the user*/
...
/**
 * \brief The list of removed elements in the library composite. This will only
 *        return elements that are of one of the types specified in the
 *        elementTypesFilter startup option for the given delegate.
 *
 * \param delegate The delegate which is requesting the removed elements.
 *
 * \return An array that contains the removed elements in the library composite.
 */
- (NSArray *)removedElements:(id<AdobeLibraryDelegate>)delegate;

/**
 * \brief Remove the given element from the library composite. This will also remove all representations
 * for this element from the library composite if it is permanently removed. If it is not permanently
 * removed, it will be placed in the removedElements array of the library.
 *
 * \param element           The element to remove.
 * \param permanentlyRemove Whether to permanently remove the element or move it to the removedElements array.
 * \param errorPtr          Gets set if an error occurs while removing the element.
 *
 * \return the removed element
 */
- (AdobeLibraryElement *)removeElement:(AdobeLibraryElement *)element permanentlyRemove:(BOOL)permanentlyRemove andError:(NSError **)errorPtr;

/**
 * \brief Remove the given element and moves it to the removedElements array of the library composite.
 *
 * \param element     The element to remove.
 * \param errorPtr    Gets set if an error occurs while removing the element.
 *
 * \return the removed element
 *
 * \note This method has been deprecated, use removeElement:permanentlyRemove:andError from above.
 */
- (AdobeLibraryElement *)removeElement:(AdobeLibraryElement *)element andError:(NSError **)errorPtr __deprecated;

/**
 * \brief Restores the given element from the removedElements array of the library composite.
 *
 ...
 */
- (AdobeLibraryElement *)restoreElement:(AdobeLibraryElement *)element andError:(NSError **)errorPtr;
 ...
 * \param representation the representation to get content URL for
...
/**
 * \brief Reverts changes to the library and sets it to the state as present in the cloud.
 *
 * \param handlerQueue	  Optional parameter. If not nil determines the operation queue completion and error blocks get executed on.
 * \param completionBlock The block that gets called in case of success.
 * \param errorBlock      The block that gets called with the error object in case of error.
 *
 * \return YES on success/no changes required; NO if a file system error occurs or if this library is local.
 *
 * \note An application that calls this method must be careful to ensure that the library is not used,
 * and until after the completion handler is called.
 *
 */
- (void)revertUnsyncedChanges:(NSOperationQueue *)handlerQueue onComplete:(void (^)(void))completionBlock onError:(void (^)(NSError *error))errorBlock;
