 ...
 *
 * [Adobe360MessageBuilder addOutputAsset:withError:]
 * [Adobe360MessageBuilder responseContext]
 *
 * Adobe360Context
 */
 ...
 *                the status bar.
 */
- (void)setRoundtripEnabled:(BOOL)enabled __deprecated_msg("Use - (void)enableSave:(BOOL)enableSave withAnimation:(BOOL)animate instead");

/**
 * Set to enable the save option in the status bar. By default this is YES.
 * Apps that need an intermediate state where they can't save & return immediately should use this option.
 *
 * @param enable Should be set to YES/NO
 * @param animate animates the label text change.
 */
- (void)enableSave:(BOOL)enable withAnimation:(BOOL)animate;
