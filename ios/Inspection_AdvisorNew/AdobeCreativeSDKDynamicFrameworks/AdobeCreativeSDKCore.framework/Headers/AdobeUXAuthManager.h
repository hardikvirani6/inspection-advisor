...
/**
 * This notification is sent through the NSNotificationManager when a user will be shown the login window using the AdobeUXAuthManager.
 */
extern NSString *const AdobeAuthManagerWillPresentLoginWindowNotification;

/**
 * This notification is sent through the NSNotificationManager when the login window will be dismissed using the AdobeUXAuthManager.
 * Note: this does not imply a successful authentication, just that the window will be dismissed.
 */
extern NSString *const AdobeAuthManagerWillDismissLoginWindowNotification;


/**
 * Clients may require additional permissions in the form of scopes.  Check your clientID documentation.
 * @see setAuthenticationParametersWithClientID:clientSecret:additionalScopeList:.
 *
 * Address/location scope.
 */
extern NSString *const AdobeAuthManagerAddressScope;

/**
 * Email scope.
 */
extern NSString *const AdobeAuthManagerEmailScope;

/**
 * Access to Typekit platform scope.
 */
extern NSString *const AdobeAuthManagerTypekitPlatformScope;

/**
 * Allow Typekit font file synchronization scope.
 */
extern NSString *const AdobeAuthManagerTypekitPlatformSyncScope;

/**
 * User profile scope.
 */
extern NSString *const AdobeAuthManagerUserProfileScope;
...
/**
 * The Social Sign In Provider ID for Facebook
 */
extern NSString * const AdobeAuthFacebookProviderID;

/**
 * The Social Sign In Provider ID for Google
 */
extern NSString * const AdobeAuthGoogleProviderID;
 ...
 *
 * clientIDs supplied by Adobe may be configured to use a clientID-specific redirectURL.  In such cases, the
 * developer must use this property to prime the Creative SDK with the provided redirectURL.  The redirectURL
 * is used internally by the Creative SDK to complete signin/signup.
 */
...
- (void)setAuthenticationParametersWithClientID:(NSString *)clientID
                            ...
                            additionalScopeList:(NSArray *)scopeList;
...
#ifndef DMALIB
/**
 * Logs in the user. This method creates a view that handles all aspects of account login and 
 * account creation.
 *
 ...
 *
 * @note This call is not explicity needed to be called. If authentication is required, it will be 
 *       called later.
 *
 * @note This method is not available to extensions. Use a shared app group and authenticate a user 
 *       through the containing app.
 */
 ...
 *
 * @param enable        Determines whether to start or stop the reporting.
 * @param completion    The block of code that gets called upon completion.
 */
- (void)sendUsageData:(BOOL)enable
