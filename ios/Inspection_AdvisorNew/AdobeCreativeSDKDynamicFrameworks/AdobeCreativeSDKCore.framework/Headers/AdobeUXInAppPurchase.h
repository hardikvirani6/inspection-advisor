 ...
 */
__deprecated_msg("Third party in-app purchase integration is no longer supported.") @interface AdobeUXInAppPurchase : NSObject

/**
 * This property indicates whether or not the SKProductViewController is being presented.
 */
@property (nonatomic, assign, getter = isPresentingProductViewController) BOOL presentingProductViewController;
...
- (BOOL)handleOpenURL:(NSURL *)url
      fromApplication:(NSString *)sourceApplication
       withCompletion:(AdobeUXInAppPurchaseBlock)completion __deprecated_msg("Use - openURL:completion: instead");

/**
 * Called in the current UIApplicationDelegate's -application:openURL:options: method
 * This method provides storage upgrade success or failure information to your application.
 *
 * @param url The @c NSURL that was passed as the URL parameter of -application:openURL:options:
 * @param completion A completion block of type AdobeUXInAppPurchaseBlock that is called upon success or failure.
 *
 * @return Returns YES if the URL can be handled by the receiver, otherwise returns NO.
 */
- (BOOL)openURL:(NSURL *)url
     completion:(AdobeUXInAppPurchaseBlock)completion;
 ...
 *
 */
- (BOOL)performStorageUpgrade __deprecated_msg("Use -performStorageUpgradeWithCompletionBlock: instead");

/**
 * Initiates a storage upgrade. If the user has the Creative Cloud Application installed, it will be opened; if not, the user will
 * be presented with the Creative Cloud's application page in a popover (iPad) or modal (iPhone/iPod) view controller.
 *
 * @param completionBlock An optional completion block that gets passed a @c BOOL to indicate whether or not the user was
 * directed to the CCMobile application. If the user chooses to cancel the store kit dialog, installation fails, or navigation
 * CCMobile fails, @c didNavigateToCCMobile will be @c NO
 *
 */
- (void)performStorageUpgradeWithCompletionBlock:(void (^)(BOOL didNavigateToCCMobile))completionBlock;
